...
def find_triggers(product_loyalty_df):
    ...
    triggers_df = product_loyalty_df.groupby("variant_id").agg(
        {'loyalty_score': 'max','order_date': 'max'}) - product_loyalty_df.groupby("variant_id").agg({'loyalty_score': 'min','order_date': 'min'})
    triggers_df = triggers_df[triggers_df["order_date"] > np.timedelta64(30, 'D')]
    triggers_df = triggers_df.rename(columns={'loyalty_score': 'difference'})[["difference"]]
